#!/usr/bin/env python3
"""Run the Shiny + auth proxy stack locally for Firebase login testing."""
...
def image_has_shiny_app(image: str) -> bool:
    result = run(
        ["docker", "run", "--rm", image, "test", "-d", "/srv/shiny-server/shinycell"],
        check=False,
    )
...
def bake_shiny_app(
    ...
    container_dataset_path = "/opt2/seurat_object.rds"
    dataset_mount = f"{dataset_path}:{container_dataset_path}:ro"
    original_cmd = get_image_cmd(image)
    script_parts = [
        "Rscript",
        "/opt2/build_shinycell.R",
        "--obj",
...
def start_containers(args: argparse.Namespace) -> None:
    ...
    if not image_has_shiny_app(args.shiny_image):
        print("[+] Shiny assets missing from image; generating via build_shinycell.R ...")
        bake_shiny_app(
            args.shiny_image,
            rds_path,
            args.project_id,
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    up_parser.add_argument("--shiny-dockerfile", default="docker/shinycell/app/Dockerfile", help="Dockerfile for the Shiny container")
    ...
    up_parser.add_argument("--shiny-image", default="shinycell-local:test", help="Tag for the local Shiny image")
    ...
    up_parser.add_argument("--shiny-container", default="shinycell-local", help="Container name for the Shiny service")
    ...
    down_parser.add_argument("--shiny-container", default="shinycell-local", help="Container name for the Shiny service")
